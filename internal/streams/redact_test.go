...
func TestRedact(t *testing.T) {
	...
	// Init fake redirect.
	RedirectFunc("ffmpeg", func(url string) (string, error) { return url[7:], nil })
	...
	assert.Equal(
		"ffmpeg:rtsp://:xxxxx@localhost:8554",
		Redact("ffmpeg:rtsp://:password@localhost:8554"),
	)
	...
	assert.Equal(
		"ffmpeg:rtsp://:xxxxx@localhost:8554?foo=bar&token=xxxxx#refresh_token=xxxxx#timeout=30",
		Redact("ffmpeg:rtsp://:password@localhost:8554?foo=bar&token=foo#timeout=30#refresh_token=baz"),
	)
